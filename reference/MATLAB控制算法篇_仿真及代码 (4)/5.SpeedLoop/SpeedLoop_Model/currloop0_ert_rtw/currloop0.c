...
void currloop0_step(void)
{
  ...
  if (rtDW.is_active_c3_currloop0 == 0U) {
    rtDW.is_active_c3_currloop0 = 1U;
    rtDW.is_c3_currloop0 = IN_IDLE;
  } else {
    switch (rtDW.is_c3_currloop0) {
     case IN_AlignStage:
      if (rtDW.temporalCounter_i1 >= 2000) {
        rtDW.is_c3_currloop0 = IN_OpenStage;
     ...
     case IN_IDLE:
      if (rtU.Motor_OnOff == 1.0F) {
        rtDW.is_c3_currloop0 = IN_AlignStage;
        rtDW.temporalCounter_i1 = 0U;
      } else {
     ...
     case IN_OpenStage:
      if (rtU.Motor_OnOff == 0.0F) {
        ...
        rtDW.is_c3_currloop0 = IN_RunStage;
      } else {
        if (rtDW.cnt == 1.0) {
          rtDW.ZReset = 1.0;
        }
   ...
   *  Sum: '<S9>/Add'
   */
  rtb_Sum1 = 0.0F - (rtb_Integrator * rtb_SinCos1 + rtb_DiscreteTimeIntegrator *
                     rtb_SinCos);
   ...
   */
  rtb_SignPreSat = rtb_Sum1 * 0.26F + (real32_T)rtDW.Integrator_DSTATE_p *
    0.0001F;
   ...
   *  Sum: '<S9>/Add1'
   */
  rtb_DiscreteTimeIntegrator = rtDW.Merge1 - (rtb_DiscreteTimeIntegrator *
    rtb_SinCos1 - rtb_Integrator * rtb_SinCos);
   ...
   */
  rtb_Integrator = rtb_DiscreteTimeIntegrator * 0.26F + rtDW.Integrator_DSTATE;
...
void currloop0_initialize(void)
{
  /* Registration code */
  ...
  /* SystemInitialize for Atomic SubSystem: '<Root>/currloop' */
  ...
  /* End of SystemInitialize for SubSystem: '<Root>/currloop' */
}
